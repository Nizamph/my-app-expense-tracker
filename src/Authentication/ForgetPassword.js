      ...
      <div className={styles.signUp}>Forget Password??</div>
       <div className={styles.forgetDescription}>
        ...
        <form className={styles.form} onSubmit={forgetPasswordHandler}>
         ...
         {!isLoading && <button className={styles.signUpbtn} >Continue</button>}
         {isLoading && <p style={{color:"white",textAlign:"center"}} >sending...</p>}
         {isCheckout && <p style={{color:"white",textAlign:"center"}} >please Checkout Your Email</p>}
        </form>
      </div>
      <Link to="/Login" style={{textDecoration:"none",color:"white",marginTop:"7px"}}>Do you have an account? Log in</Link>
    </div>
    </div>
